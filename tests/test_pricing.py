...
def test_wider_uncertainty_never_tightens_spread():
    ...
    assert wide.components["model_uncertainty_bps"] > tight.components["model_uncertainty_bps"]
    # more legs -> more uncertainty
    three = _legs() + [LegMarkInput(symbol="IND", side="YES", bid=0.08,
                                    ask=0.10, bid_size=500.0, ask_size=500.0)]
    d3 = price_combo(three, rfq_id="R", qty_decimal="100")
    # fair shrinks with the extra leg, so compare relative spread (bps):
    # more legs must never tighten it.
    assert d3.components["spread_bps_total"] >= tight.components["spread_bps_total"]
    assert d3.components["model_uncertainty_bps"] > tight.components["model_uncertainty_bps"]


def test_bigger_size_never_tightens_spread():
    small = price_combo(_legs(), rfq_id="R", qty_decimal="10")
    big = price_combo(_legs(), rfq_id="R", qty_decimal="100000")
    assert big.components["depth_impact_bps"] >= small.components["depth_impact_bps"]
    assert big.half_spread >= small.half_spread


def test_midpoint_fallback_without_sizes():
    legs = [LegMarkInput(symbol="A", side="YES", bid=0.4, ask=0.6),
            LegMarkInput(symbol="B", side="NO", bid=0.4, ask=0.6)]
...
def test_resolved_legs():
    ...
    assert d.reason_code == QUOTED_OK and abs(d.fair - 1.0) < 1e-9
...
def test_expected_edge_positive_and_explained():
    ...
    comps = d.components
    for key in ("base_edge_bps", "model_uncertainty_bps", "depth_impact_bps",
                "event_risk_bps", "operational_buffer_bps", "spread_bps_total",
                "half_spread", "leg_marks"):
        assert key in comps
    total = (comps["base_edge_bps"] + comps["model_uncertainty_bps"]
             + comps["depth_impact_bps"] + comps["event_risk_bps"]
             + comps["operational_buffer_bps"])
    assert abs(total - comps["spread_bps_total"]) < 1e-9
